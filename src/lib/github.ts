...
export const getProjects = async (perPage = 10) => {
  ...
  // Filter out repositories without a primary language (those that don't have code)
  const repositoriesWithCode = data.data.user.repositories.nodes.filter(
    (repo) => repo.primaryLanguage !== null
  );

  return repositoriesWithCode.map(
    (repo) =>
      ({
        name: repo.name,
        url: repo.url,
