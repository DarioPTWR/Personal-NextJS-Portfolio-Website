      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            author: {
              '@type': 'Person',
              name: 'Dario Prawara Teh',
            },
          }),
        }}
      />
      <h1 className="mb-4 ml-0.5 text-4xl font-bold tracking-tight text-black dark:text-white md:text-5xl md:ml-0">
        <Balancer>{post.title}</Balancer>
      </h1>
      <div className="mt-2 flex w-full flex-col items-start justify-between sm:flex-row sm:items-center">
        <div className="flex items-center">
          <p className="ml-0.5 text-sm text-gray-700 dark:text-[#c2c2c2]">
        ...
        <p className="mt-2 text-sm text-gray-600 dark:text-[#c2c2c2] md:mt-0">
      ...
      <div className="mt-2 flex w-full text-xs">
        {post.tags?.map((tag) => (
          <Link
            ...
            key={tag}
            className={clsx(
              'mr-2 mt-2 rounded-full border border-indigo-500 px-2.5 py-0.5 text-xs text-indigo-500',
